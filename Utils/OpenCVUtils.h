#ifndef OpenCVUtils_h__
#define OpenCVUtils_h__
#include <fstream>
#include <ostream>
...
*/
static inline bool SaveMatBinary(const std::string& filename, const cv::Mat& output){
...
*/
static inline bool readMatBinary(std::ifstream& ifs, cv::Mat& in_mat)
{
	if (!ifs.is_open()){
		return false;
	}
...
*/
static inline bool LoadMatBinary(const std::string& filename, cv::Mat& output){
	std::ifstream ifs(filename, std::ios::binary);
	return readMatBinary(ifs, output);
}
#endif // OpenCVUtils_h__
