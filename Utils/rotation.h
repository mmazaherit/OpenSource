...
namespace ceres
{
		...
		void AngleAxisTransformPoint(const T angle_axis_translation[6], const T pt[3], T result[3])
		{
		...
		template <typename T>
		inline void EulerAnglesToRzRyRx(const T* euler,
		...
		template <typename T>
		inline void EulerAnglesToMzMyMx(const T* euler,
			const int row_stride_parameter,
			T* R) {
			CHECK_EQ(row_stride_parameter, 3);
		...
		template <typename T>
		inline void RotationMzMyMxToEuler( const int row_stride_parameter,const T* M , T*euler) {
			CHECK_EQ(row_stride_parameter, 3);
			EulerAnglesToMzMyMx(RowMajorAdapter3x3(M), euler);
		}

		template <typename T, int row_stride, int col_stride>
		inline void	RotationMzMyMxToEuler(const MatrixAdapter<T, row_stride, col_stride>& M, T* euler)
		{
			const double kPi = 3.14159265358979323846;
		...
		template <typename T>
		inline void TransposeRotation(const T* const R, T* Rt)
		{
			Rt[0] = R[0];
			Rt[1] = R[3];
			Rt[2] = R[6];
		...
		template <typename T>
		inline void RotatePoint(const T* const R, const T* const Pt, T* RotatedPoint)
		{
		...
		template <typename T>
		inline void NegateVector3(T* vec)
		{
			vec[0] = -vec[0];
			vec[1] = -vec[1];
			vec[2] = -vec[2];
